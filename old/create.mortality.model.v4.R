...
write.table(summary(morat)$conf.int,"~/Desktop/Lung cancer/lrisk/other/lifeyearsgained/HR.csv",sep=",")
...
morat_nocomorbid <- svycoxph(Surv(age, deathage, died) ~ 
                               ...
                               I(log(qtyears + 1)) + I(log(cpd + 1)) + I(sqrt(pkyr.cat)),
                  design=design, data = nhis, model = TRUE, y = TRUE) 
write.table(summary(morat_nocomorbid)$conf.int,"~/Desktop/Lung cancer/lrisk/other/lifeyearsgained/HR_nocomorbid.csv",sep=",")

save(morat, file="~/Desktop/Lung cancer/lrisk/other/lifeyearsgained/mortality.model.v4.RData")
